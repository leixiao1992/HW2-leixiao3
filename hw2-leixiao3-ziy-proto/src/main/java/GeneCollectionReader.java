...
/**
 * This is a collectionreader of the system.this process is to deal with the
 * document.First reading the document.And then store the sentence of the
 * document into the cas, with form of Sentence.In addition, we should separate
 * the sentence into id and text.The method of separation is to identify the
 * location of the space.From beginning to space is id.From space to end is
 * text.
 * 
 * @author leixiao
 *
...
public class GeneCollectionReader extends CollectionReader_ImplBase {
	...
	public void initialize() throws ResourceInitializationException {
		...
		/**
		 * To identify if the document has been read. Since it has only one
		 * document to read, we need to close the system when completing one
		 * document
		 */
		flag = 1;
